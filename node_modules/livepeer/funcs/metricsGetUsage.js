...
exports.metricsGetUsage = metricsGetUsage;
 ...
 * Query usage metrics
 */
async function metricsGetUsage(client$, request, options) {
    var _a;
    const input$ = request;
    const parsed$ = schemas$.safeParse(input$, (value$) => operations.GetUsageMetricsRequest$outboundSchema.parse(value$), "Input validation failed");
    if (!parsed$.ok) {
        return parsed$;
    }
    const payload$ = parsed$.value;
    const body$ = null;
    const path$ = (0, url_js_1.pathToFunc)("/data/usage/query")();
    ...
    const [result$] = await m$.match(m$.json(200, operations.GetUsageMetricsResponse$inboundSchema, {
        key: "usage-metric",
    }), m$.fail(["4XX", "5XX"]), m$.json("default", operations.GetUsageMetricsResponse$inboundSchema, {
        key: "error",
...
//# sourceMappingURL=metricsGetUsage.js.map
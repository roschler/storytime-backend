...
exports.metricsGetCreatorViewership = metricsGetCreatorViewership;
 ...
 */
async function metricsGetCreatorViewership(client$, request, options) {
    var _a;
    const input$ = request;
    const parsed$ = schemas$.safeParse(input$, (value$) => operations.GetCreatorViewershipMetricsRequest$outboundSchema.parse(value$), "Input validation failed");
    if (!parsed$.ok) {
        return parsed$;
    }
    const payload$ = parsed$.value;
    const body$ = null;
    const path$ = (0, url_js_1.pathToFunc)("/data/views/query/creator")();
    ...
    const [result$] = await m$.match(m$.json(200, operations.GetCreatorViewershipMetricsResponse$inboundSchema, {
        key: "data",
    }), m$.fail(["4XX", "5XX"]), m$.json("default", operations.GetCreatorViewershipMetricsResponse$inboundSchema, { key: "error" }))(response, { extraFields: responseFields$ });
    if (!result$.ok) {
        return result$;
    }
    return result$;
}
//# sourceMappingURL=metricsGetCreatorViewership.js.map
 ...
 * Retrieve a session
 */
async function sessionGet(client$, id, options) {
    var _a;
    const input$ = {
        id: id,
    };
    const parsed$ = schemas$.safeParse(input$, (value$) => operations.GetSessionRequest$outboundSchema.parse(value$), "Input validation failed");
    if (!parsed$.ok) {
        return parsed$;
    }
    ...
    const path$ = (0, url_js_1.pathToFunc)("/session/{id}")(pathParams$);
    ...
    const [result$] = await m$.match(m$.json(200, operations.GetSessionResponse$inboundSchema, {
        key: "session",
    }), m$.fail(["4XX", "5XX"]), m$.json("default", operations.GetSessionResponse$inboundSchema, {
        key: "error",
...
//# sourceMappingURL=sessionGet.js.map
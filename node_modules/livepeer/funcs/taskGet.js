 ...
 * Retrieve a Task
 */
async function taskGet(client$, taskId, options) {
    var _a;
    const input$ = {
        taskId: taskId,
    };
    const parsed$ = schemas$.safeParse(input$, (value$) => operations.GetTaskRequest$outboundSchema.parse(value$), "Input validation failed");
    if (!parsed$.ok) {
        return parsed$;
    }
    ...
    const body$ = null;
    const pathParams$ = {
        taskId: (0, encodings_js_1.encodeSimple)("taskId", payload$.taskId, {
            explode: false,
            charEncoding: "percent",
        }),
    };
    const path$ = (0, url_js_1.pathToFunc)("/task/{taskId}")(pathParams$);
    ...
    const [result$] = await m$.match(m$.json(200, operations.GetTaskResponse$inboundSchema, { key: "task" }), m$.fail(["4XX", "5XX"]), m$.nil("default", operations.GetTaskResponse$inboundSchema))(response, { extraFields: responseFields$ });
    if (!result$.ok) {
        return result$;
    }
    return result$;

...
exports.roomStopEgress = roomStopEgress;
 ...
 */
async function roomStopEgress(client$, id, options) {
    var _a;
    const input$ = {
        id: id,
    };
    const parsed$ = schemas$.safeParse(input$, (value$) => operations.StopRoomEgressRequest$outboundSchema.parse(value$), "Input validation failed");
    if (!parsed$.ok) {
        return parsed$;
    }
    ...
    const [result$] = await m$.match(m$.nil(204, operations.StopRoomEgressResponse$inboundSchema), m$.fail(["4XX", "5XX"]), m$.json("default", operations.StopRoomEgressResponse$inboundSchema, {
        key: "error",
...
//# sourceMappingURL=roomStopEgress.js.map
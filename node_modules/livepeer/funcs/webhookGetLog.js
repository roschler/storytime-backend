...
exports.webhookGetLog = webhookGetLog;
 ...
 */
async function webhookGetLog(client$, id, logId, options) {
    var _a;
    const input$ = {
        id: id,
        logId: logId,
    };
    const parsed$ = schemas$.safeParse(input$, (value$) => operations.GetWebhookLogRequest$outboundSchema.parse(value$), "Input validation failed");
    if (!parsed$.ok) {
        return parsed$;
    }
    ...
    const path$ = (0, url_js_1.pathToFunc)("/webhook/{id}/log/{logId}")(pathParams$);
    ...
    const [result$] = await m$.match(m$.json(200, operations.GetWebhookLogResponse$inboundSchema, {
        key: "webhook-log",
    }), m$.fail(["4XX", "5XX"]), m$.json("default", operations.GetWebhookLogResponse$inboundSchema, {
        key: "error",
...
//# sourceMappingURL=webhookGetLog.js.map
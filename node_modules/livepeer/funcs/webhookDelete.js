...
exports.webhookDelete = webhookDelete;
 ...
 * Delete a webhook
 */
async function webhookDelete(client$, id, options) {
    var _a;
    const input$ = {
        id: id,
    };
    const parsed$ = schemas$.safeParse(input$, (value$) => operations.DeleteWebhookRequest$outboundSchema.parse(value$), "Input validation failed");
    if (!parsed$.ok) {
        return parsed$;
    }
    ...
    const [result$] = await m$.match(m$.json(200, operations.DeleteWebhookResponse$inboundSchema, {
        key: "webhook",
    }), m$.fail(["4XX", "5XX"]), m$.json("default", operations.DeleteWebhookResponse$inboundSchema, {
        key: "error",

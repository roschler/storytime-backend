 ...
 */
async function roomGet(client$, id, options) {
    var _a;
    const input$ = {
        id: id,
    };
    const parsed$ = schemas$.safeParse(input$, (value$) => operations.GetRoomRequest$outboundSchema.parse(value$), "Input validation failed");
    if (!parsed$.ok) {
        return parsed$;
    }
    ...
    const [result$] = await m$.match(m$.json(200, operations.GetRoomResponse$inboundSchema, { key: "room" }), m$.fail(["4XX", "5XX"]), m$.json("default", operations.GetRoomResponse$inboundSchema, {
        key: "error",
...
//# sourceMappingURL=roomGet.js.map
...
exports.streamDelete = streamDelete;
 ...
 */
async function streamDelete(client$, id, options) {
    var _a;
    const input$ = {
        id: id,
    };
    const parsed$ = schemas$.safeParse(input$, (value$) => operations.DeleteStreamRequest$outboundSchema.parse(value$), "Input validation failed");
    if (!parsed$.ok) {
        return parsed$;
    }
    ...
    const [result$] = await m$.match(m$.nil(204, operations.DeleteStreamResponse$inboundSchema), m$.fail(["4XX", "5XX"]), m$.json("default", operations.DeleteStreamResponse$inboundSchema, {
        key: "error",
...
//# sourceMappingURL=streamDelete.js.map
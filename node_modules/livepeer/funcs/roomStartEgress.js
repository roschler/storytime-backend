...
exports.roomStartEgress = roomStartEgress;
 ...
 */
async function roomStartEgress(client$, id, roomEgressPayload, options) {
    var _a;
    const input$ = {
        id: id,
        roomEgressPayload: roomEgressPayload,
    };
    const parsed$ = schemas$.safeParse(input$, (value$) => operations.StartRoomEgressRequest$outboundSchema.parse(value$), "Input validation failed");
    if (!parsed$.ok) {
        return parsed$;
    }
    const payload$ = parsed$.value;
    const body$ = (0, encodings_js_1.encodeJSON)("body", payload$["room-egress-payload"], {
        explode: true,
    });
    const pathParams$ = {
    ...
    const path$ = (0, url_js_1.pathToFunc)("/room/{id}/egress")(pathParams$);
    ...
    const [result$] = await m$.match(m$.nil(204, operations.StartRoomEgressResponse$inboundSchema), m$.fail(["4XX", "5XX"]), m$.json("default", operations.StartRoomEgressResponse$inboundSchema, {
        key: "error",
...
//# sourceMappingURL=roomStartEgress.js.map
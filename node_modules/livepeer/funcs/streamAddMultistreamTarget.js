...
exports.streamAddMultistreamTarget = streamAddMultistreamTarget;
 ...
 */
async function streamAddMultistreamTarget(client$, id, targetAddPayload, options) {
    var _a;
    const input$ = {
        id: id,
        targetAddPayload: targetAddPayload,
    };
    const parsed$ = schemas$.safeParse(input$, (value$) => operations.AddMultistreamTargetRequest$outboundSchema.parse(value$), "Input validation failed");
    if (!parsed$.ok) {
        return parsed$;
    }
    const payload$ = parsed$.value;
    const body$ = (0, encodings_js_1.encodeJSON)("body", payload$["target-add-payload"], {
        explode: true,
    });
    const pathParams$ = {
    ...
    const path$ = (0, url_js_1.pathToFunc)("/stream/{id}/create-multistream-target")(pathParams$);
    ...
    const [result$] = await m$.match(m$.nil(204, operations.AddMultistreamTargetResponse$inboundSchema), m$.fail(["4XX", "5XX"]), m$.json("default", operations.AddMultistreamTargetResponse$inboundSchema, {
        key: "error",
...
//# sourceMappingURL=streamAddMultistreamTarget.js.map
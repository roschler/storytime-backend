...
exports.multistreamDelete = multistreamDelete;
 ...
 */
async function multistreamDelete(client$, id, options) {
    var _a;
    const input$ = {
        id: id,
    };
    const parsed$ = schemas$.safeParse(input$, (value$) => operations.DeleteMultistreamTargetRequest$outboundSchema.parse(value$), "Input validation failed");
    if (!parsed$.ok) {
        return parsed$;
    }
    ...
    const [result$] = await m$.match(m$.nil(204, operations.DeleteMultistreamTargetResponse$inboundSchema), m$.fail(["4XX", "5XX"]), m$.json("default", operations.DeleteMultistreamTargetResponse$inboundSchema, { key: "error" }))(response, { extraFields: responseFields$ });
    if (!result$.ok) {
        return result$;
    }
    return result$;
}
//# sourceMappingURL=multistreamDelete.js.map
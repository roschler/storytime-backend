 ...
 * Update a stream
 */
async function streamUpdate(client$, id, streamPatchPayload, options) {
    var _a;
    const input$ = {
        id: id,
        streamPatchPayload: streamPatchPayload,
    };
    const parsed$ = schemas$.safeParse(input$, (value$) => operations.UpdateStreamRequest$outboundSchema.parse(value$), "Input validation failed");
    if (!parsed$.ok) {
        return parsed$;
    }
    const payload$ = parsed$.value;
    const body$ = (0, encodings_js_1.encodeJSON)("body", payload$["stream-patch-payload"], {
        explode: true,
    });
    const pathParams$ = {
    ...
    const path$ = (0, url_js_1.pathToFunc)("/stream/{id}")(pathParams$);
    ...
    const [result$] = await m$.match(m$.nil(204, operations.UpdateStreamResponse$inboundSchema), m$.fail(["4XX", "5XX"]), m$.json("default", operations.UpdateStreamResponse$inboundSchema, {
        key: "error",
...
//# sourceMappingURL=streamUpdate.js.map
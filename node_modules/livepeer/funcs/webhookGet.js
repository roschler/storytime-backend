 ...
 * Retrieve a webhook
 */
async function webhookGet(client$, id, options) {
    var _a;
    const input$ = {
        id: id,
    };
    const parsed$ = schemas$.safeParse(input$, (value$) => operations.GetWebhookRequest$outboundSchema.parse(value$), "Input validation failed");
    if (!parsed$.ok) {
        return parsed$;
    }
    ...
    const [result$] = await m$.match(m$.json(200, operations.GetWebhookResponse$inboundSchema, {
        key: "webhook",
    }), m$.fail(["4XX", "5XX"]), m$.json("default", operations.GetWebhookResponse$inboundSchema, {
        key: "error",
...
//# sourceMappingURL=webhookGet.js.map
 ...
 */
async function streamCreate(client$, request, options) {
    var _a;
    const input$ = request;
    const parsed$ = schemas$.safeParse(input$, (value$) => components.NewStreamPayload$outboundSchema.parse(value$), "Input validation failed");
    if (!parsed$.ok) {
        return parsed$;
    }
    ...
    const [result$] = await m$.match(m$.json(201, operations.CreateStreamResponse$inboundSchema, {
        key: "stream",
    }), m$.fail(["4XX", "5XX"]), m$.json("default", operations.CreateStreamResponse$inboundSchema, {
        key: "error",
...
//# sourceMappingURL=streamCreate.js.map
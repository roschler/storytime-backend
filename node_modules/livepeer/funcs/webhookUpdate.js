 ...
 * Update a webhook
 */
async function webhookUpdate(client$, id, webhook, options) {
    var _a;
    const input$ = {
        id: id,
        webhook: webhook,
    };
    const parsed$ = schemas$.safeParse(input$, (value$) => operations.UpdateWebhookRequest$outboundSchema.parse(value$), "Input validation failed");
    if (!parsed$.ok) {
        return parsed$;
    }
    const payload$ = parsed$.value;
    const body$ = (0, encodings_js_1.encodeJSON)("body", payload$.webhook, { explode: true });
    const pathParams$ = {
    ...
    const path$ = (0, url_js_1.pathToFunc)("/webhook/{id}")(pathParams$);
    ...
    const [result$] = await m$.match(m$.json(200, operations.UpdateWebhookResponse$inboundSchema, {
        key: "webhook",
    }), m$.fail(["4XX", "5XX"]), m$.json("default", operations.UpdateWebhookResponse$inboundSchema, {
        key: "error",

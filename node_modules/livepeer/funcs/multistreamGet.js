...
exports.multistreamGet = multistreamGet;
 ...
 */
async function multistreamGet(client$, id, options) {
    var _a;
    const input$ = {
        id: id,
    };
    const parsed$ = schemas$.safeParse(input$, (value$) => operations.GetMultistreamTargetRequest$outboundSchema.parse(value$), "Input validation failed");
    if (!parsed$.ok) {
        return parsed$;
    }
    ...
    const [result$] = await m$.match(m$.json(200, operations.GetMultistreamTargetResponse$inboundSchema, {
        key: "multistream-target",
    }), m$.fail(["4XX", "5XX"]), m$.json("default", operations.GetMultistreamTargetResponse$inboundSchema, {
        key: "error",
...
//# sourceMappingURL=multistreamGet.js.map
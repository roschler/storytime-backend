...
exports.streamStartPull = streamStartPull;
 ...
 * started.
 */
async function streamStartPull(client$, id, options) {
    var _a;
    const input$ = {
        id: id,
    };
    const parsed$ = schemas$.safeParse(input$, (value$) => operations.StartPullStreamRequest$outboundSchema.parse(value$), "Input validation failed");
    if (!parsed$.ok) {
        return parsed$;
    }
    ...
    const path$ = (0, url_js_1.pathToFunc)("/stream/{id}/start-pull")(pathParams$);
    ...
    const [result$] = await m$.match(m$.nil(204, operations.StartPullStreamResponse$inboundSchema), m$.fail(["4XX", "5XX"]), m$.json("default", operations.StartPullStreamResponse$inboundSchema, {
        key: "error",
...
//# sourceMappingURL=streamStartPull.js.map
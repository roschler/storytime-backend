 ...
 * Retrieve a stream
 */
async function streamGet(client$, id, options) {
    var _a;
    const input$ = {
        id: id,
    };
    const parsed$ = schemas$.safeParse(input$, (value$) => operations.GetStreamRequest$outboundSchema.parse(value$), "Input validation failed");
    if (!parsed$.ok) {
        return parsed$;
    }
    ...
    const [result$] = await m$.match(m$.json(200, operations.GetStreamResponse$inboundSchema, { key: "stream" }), m$.fail(["4XX", "5XX"]), m$.json("default", operations.GetStreamResponse$inboundSchema, {
        key: "error",
...
//# sourceMappingURL=streamGet.js.map